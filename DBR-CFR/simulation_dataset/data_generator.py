...
from simulation_dataset.csv2npz import np2npz
from simulation_dataset.plotting import draw_w
...
def get_multivariate_normal_params(m,mA,mB,mC, seed=0, dep=0):
    np.random.seed(seed)

    if dep:
        mu = np.random.normal(size=m) / 10.
        ''' sample random positive semi-definite matrix for cov '''
        temp = np.random.uniform(size=(m, m))*10
...
def get_latent(m,mA,mB,mC, seed, n, dep):
	L = np.array((n*[[]]))
	if m != 0:
		mu, sig = get_multivariate_normal_params(m,mA,mB,mC, seed, dep)
...
def run(run_dict, size=0):
    ...
    # Variables
    temp = get_latent(max_dim, mA, mB, mC, seed_coef * init_seed + 4, n_trn, dep)
    ...
    # A = get_latent(mA, seed_coef * init_seed + 1, n_trn, dep)*2
    # B = get_latent(mB, seed_coef * init_seed + 2, n_trn, dep)*4
    # C = get_latent(mC, seed_coef * init_seed + 3, n_trn, dep)*6
    # D = get_latent(mD, seed_coef * init_seed + 4, n_trn, dep)
    ...
    # base mu_0, mu_1
    if random_coef == "True" or random_coef == "T" or use_one == "True" or use_one == "T":   
        mu_0 = np.dot(BC ** 1+0.5, coefs_2) / (mB + mC)
        mu_1 = np.dot(BC ** 2, coefs_3) / (mB + mC)
    else:
        mu_0 = np.dot(BC ** 3+0.5, coefs_2) / (mB + mC) / run_dict["coef_devide_2"]
    ...
    if binary == "False" or binary == "F":
        ...
        print(np.mean(mu_0), np.mean(y[:,0]))
        # np.random.seed(3 * seed_coef * init_seed)  # <--
        y[:, 1] = mu_1 + np.random.normal(loc=0., scale=.1, size=n_trn)
        print(np.mean(mu_1), np.mean(y[:,1]))
    ...
    print(np.mean(yf),yf.shape)
    ##################################################################
    # data_path = storage_path+'/data/'+which_benchmark
    data_path = storage_path + '/simulation_dataset/'

    if not os.path.exists(storage_path + '/simulation_dataset/'):
        os.mkdir(storage_path + '/simulation_dataset/')
    ...
    with open(data_path + file_cont_name + '.csv', 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=',')
        for i in np.random.permutation(n_trn):
            temp = [t[i], y_cont[i], mu_0[i], mu_1[i], mu_0[i] + t[i] * mu_1[i]]
            temp.extend(x[i, :])
            csv_writer.writerow(temp)
