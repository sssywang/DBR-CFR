...
def np2npz(file_path, num = 10):
    ...
    np.savez(file_path + 'Syn.train.npz', mu1=mu1s, mu0=mu0s, yf=yfs, ycf=ycfs, t=ts, x=xs, e=es, I_trains=I_trains, I_valids=I_valids)
    np.savez(file_path + 'Syn.test.npz', mu1=t_mu1s, mu0=t_mu0s, yf=t_yfs, ycf=t_ycfs, t=t_ts, x=t_xs, e=t_es)
