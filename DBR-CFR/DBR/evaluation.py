...
def evaluate_cont_ate(predictions, data, i_exp, I_subset=None,
    ...
    # N = t.shape[0]
    # y0_pre = yf_p * (1 - t) + ycf_p * t
    # y1_pre = yf_p * t + ycf_p * (1 - t)
    # idx_treat = np.where(t==1)[0]; idx_control = np.where(t==0)[0]
    # yf_1 = yf[idx_treat]; yf_0 = yf[idx_control]
    # yf_1_pre = y1_pre[idx_treat]; yf_0_pre = y0_pre[idx_control]
    # tpre_1 = tpre[idx_treat]; tpre_0 = tpre[idx_control]
    # theta1_dml = compute_theta.compute_dml_theta(y1_pre, yf_1, yf_1_pre, tpre_1)
    # theta0_dml = compute_theta.compute_dml_theta(y0_pre, yf_0, yf_0_pre, tpre_0)
    # ate_dml = theta1_dml - theta0_dml
    #
    # res_m = t-tpre
    # y1_minus_g1_given_z_list = compute_theta.prepare_set_A(idx_treat, yf, y1_pre, N, 1000)
    # y0_minus_g0_given_z_list = compute_theta.prepare_set_A(idx_control, yf, y0_pre, N, 1000)
    # E_res_m_list = compute_theta.compute_E_max_r(res_m, 2)
    # theta1_rcl = compute_theta.compute_rcl_theta(y1_pre, res_m, y1_minus_g1_given_z_list, 2, 2, E_res_m_list)
    # theta0_rcl = compute_theta.compute_rcl_theta(y0_pre, res_m, y0_minus_g0_given_z_list, 2, 2, E_res_m_list)
    # ate_rcl = theta1_rcl - theta0_rcl
    return {'ate_pred': ate_pred, 'att_pred': att_pred,
            'atc_pred': atc_pred, 'bias_ate': bias_ate, 'bias_ate_dml':np.nan, 'bias_ate_rcl':np.nan,
            'bias_att': bias_att, 'bias_atc': bias_atc,
            'rmse_fact': rmse_fact, 'rmse_cfact': rmse_cfact,
            'pehe': pehe, 'rmse_ite': rmse_ite, 'pehe_nn': pehe_appr, 'auc': auc,'roc_auc':roc_auc,
                'fact_auc':fact_auc,'fact_roc_auc':fact_roc_auc}
...
def evaluate(output_dir, data_path_train, data_path_test=None, binary=False):
    ...
    multiple_exps = (configs[0]['experiments_end']-configs[0]['experiments_start'] > 1)
...
def eval_ate(result, data_train, data_test, i_exp, i_sel):
    ate_dml_train, ate_rcl_train = compute_ate(result['train'], data_train, i_exp, i_sel, validation=False)
    ate_dml_valid, ate_rcl_valid = compute_ate(result['train'], data_train, i_exp, i_sel, validation=True)
    ate_dml_test, ate_rcl_test = compute_ate(result['test'], data_test, i_exp, i_sel, validation=False)
    return ate_dml_train, ate_rcl_train, ate_dml_valid, ate_rcl_valid, ate_dml_test, ate_rcl_test
def compute_ate(result, data, i_exp, i_sel, validation):
    predictions = result['pred']
    tpre = result['tpre']
    predictions = predictions[:,:,i_exp,i_sel]
    tpre = tpre[:, :, i_exp, i_sel]
    yf_p = predictions[:, 0]
    ycf_p = predictions[:, 1]
    tpre = tpre[:, 0]
    t = data['t'][:,i_exp]
    ...
    y0_pre = yf_p * (1 - t) + ycf_p * t
    y1_pre = yf_p * t + ycf_p * (1 - t)
    idx_treat = np.where(t == 1)[0];
    idx_control = np.where(t == 0)[0]
    yf_1 = yf[idx_treat];
    yf_0 = yf[idx_control]
    yf_1_pre = y1_pre[idx_treat];
    yf_0_pre = y0_pre[idx_control]
    tpre_1 = tpre[idx_treat];
    tpre_0 = tpre[idx_control]
    theta1_dml = compute_theta.compute_dml_theta(y1_pre, yf_1, yf_1_pre, tpre_1)
    theta0_dml = compute_theta.compute_dml_theta(y0_pre, yf_0, yf_0_pre, tpre_0)
    ate_dml = theta1_dml - theta0_dml
    ...
    bias_ate_dml = true_ate - ate_dml
    bias_ate_rcl = true_ate - ate_rcl
    return bias_ate_dml, bias_ate_rcl